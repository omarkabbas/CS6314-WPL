...
def getTodoItemById():
    try:
        if session.get('user'):
            ...
            todoitem = []
            todoitem.append({'id':result[0][0],'title':result[0][1],'description':result[0][2],'isComplete':result[0][4]})
...
def updateTodoItem():
    try:
        if session.get('user'):
            ...
            _completed = request.form['isComplete']
            _todoitem_id = request.form['id']
 
            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.execute("UPDATE tbl_todo set title = %s , description = %s, isComplete = %s where id = %s and userid = %s",(_title,_description,_completed,_todoitem_id,_user))
            data = cursor.fetchall()
